...
public class Stock implements Serializable{
	private ArrayList <Article> listeDArticles;
	 ...
	 /**
	 *On considère qu'à chaque emprunt, l'article perd 2 points d'intégrité.
	 *
	 */
	...
	/**
	*La m&eacute; ajoute un article au stock; équivalent à l'achat d'un article.
	*@param article indique l'article à ajouter au stock;
	*/
	 ...
	 public String toString(){		
		...
		String desc=("\n\nNombre total d'articles en stock: " + this.listeDArticles.size() + "\n" +
					 "Nombre d'articles empruntés     : " + this.articlesEmpruntes().size() + "\n" +
					 "Nombre d'articles disponibles   : " + this.articlesDisponibles().size() + "\n" +
					 "Nombre d'articles à réparer     : " + this.articlesAReparer().size() + "\n" +
					 "Nombre d'articles en réparation : " + this.articlesEnReparation().size() + "\n");	
	 ...
	 public Article verifierIdentifiant(String identifiant){
	 	...
	 	while(it.hasNext() && !trouve){
	 		...
	 		if(cpAr.getIdArticle().toLowerCase().equals(identifiant.toLowerCase())){
	 			trouve = true;
	 			ar=cpAr;
	 		}
	 	}
